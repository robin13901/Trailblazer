...
void main() {
  group('RtreeBuilder', () {
    group('perSegment (opt-in)', () {
      test('3-point line → 2 rtree rows', () {
      ...
      test('bbox arithmetic: (52.5, 13.4)→(52.6, 13.5) has exact bbox', () {
        final db = _openOsmSqliteInMemory();
        try {
          final b = RtreeBuilder(db, RtreeGranularity.perSegment);
          try {
    ...
    group('perWay (default)', () {
      test('builds 1 rtree row per way', () {
        final db = _openOsmSqliteInMemory();
        try {
          ...
          try {
            final line = <Vec2>[
              ...
              const Vec2(13.401, 52.501),
              const Vec2(13.402, 52.502),
              const Vec2(13.403, 52.503),
              const Vec2(13.404, 52.504),
            ];
            final written = b.buildForWay(42, line);
            expect(written, 1);
            expect(b.rowsWritten, 1);
            final count = db
      ...
      test('bbox spans the full polyline', () {
        final db = _openOsmSqliteInMemory();
        try {
          final b = RtreeBuilder(db, RtreeGranularity.perWay);
          try {
            final line = <Vec2>[
              ...
              const Vec2(13.402, 52.502),
              const Vec2(13.420, 52.520),
              const Vec2(13.405, 52.505),
            ];
            b.buildForWay(1, line);
            final row = db.select('SELECT * FROM ways_rtree;').first;
            expect(row['min_lat'] as double, closeTo(52.500, 1e-4));
            expect(row['max_lat'] as double, closeTo(52.520, 1e-4));
            expect(row['min_lng'] as double, closeTo(13.400, 1e-4));
            expect(row['max_lng'] as double, closeTo(13.420, 1e-4));
          } finally {
            b.dispose();
          }
        } finally {
      ...
      test('segment_idx = -1 sentinel', () {
        ...
        try {
          ...
          try {
            ...
            b.buildForWay(42, line);
            final row = db.select(
              'SELECT segment_idx FROM ways_rtree_lookup '
              'WHERE way_id = 42;',
            ).first;
      ...
      test('degenerate lines still produce 0 rows', () {
        final db = _openOsmSqliteInMemory();
        try {
          final b = RtreeBuilder(db, RtreeGranularity.perWay);
          try {
    ...
    group('loadFromMeasurement (Plan 04-10-1-03 default: perWay)', () {
      late Directory tmp;
      ...
      test('missing file → perWay default', () async {
      ...
      test('file explicitly saying per-segment → perSegment', () async {
        final f = File('${tmp.path}/perseg.md')
          ..writeAsStringSync('Recommend per-segment granularity.');
      ...
      test('file mentioning neither → perWay default', () async {
