...
- [x] **Phase 10: Coverage Recompute & Region Totals** — user-triggered full re-match + recompute (Regions tab, confirmation-gated), correct region-type badges, offline-precomputed bundled per-region total road length (zero runtime API calls), Ortsteil-level (L9) granularity; pill-nameable regions ⊆ bundled totals table. *(Replaced the former "Hardening" phase 2026-07-17 at user request — see 10-CONTEXT.md + PROJECT.md Key Decisions. QUA-01/04/07 de-scoped per plan 10-01.)* **Code-complete + verifier 6/6 CODE-COMPLETE (status: human_needed) 2026-07-17; `flutter analyze` clean, 891 tests green. TWO documented human-action checkpoints deferred: (1) PBF-gated asset regeneration — `region_totals.json.gz` + L9 admin bundle from a ~4 GB Geofabrik Germany PBF (Stage H code + parity test + key-set assertion CLI all shipped; assets not regenerated because the PBF is absent — designed halt in 10-03 autonomous-safety gate); (2) on-device confirms (recompute button end-to-end + live-puck-riding-line). See `10-VERIFICATION.md`.**
...
| 10. Coverage Recompute & Region Totals | 5/5 | ✓ Complete (verifier 6/6 CODE-COMPLETE, status human_needed; 891 tests green; 2 deferred checkpoints — PBF asset regen + on-device confirms) | 2026-07-17 |
