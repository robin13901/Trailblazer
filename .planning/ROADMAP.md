...
**Success Criteria** (what must be TRUE):
  ...
  3. A CI-runnable golden corpus of ≥ 5 recorded trips at Phase 5 close-out (autobahn, Kreisel, tunnel, parking, U-turn, city grid, roundabout, one-way — scenario diversity across the 5 seeds) growing to ≥ 20 by Phase 6 close-out; core matcher module has ≥ 90 % line coverage; regression on any golden trip fails CI. `tool/osm_pipeline/` (retained as dev-only per OSM-07) is the fixture generator for golden PBFs. **Phase 6 inherits the corpus-expansion obligation** (record + fixture-ize the remaining 15 trips alongside inbox integration).
